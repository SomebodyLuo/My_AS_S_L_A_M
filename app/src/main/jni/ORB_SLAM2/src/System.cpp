...
{
    string t_str = strVocFile;

    mpVocabulary = new ORBVocabulary();

    mpVocabulary->loadFromTextFile(t_str);

    int nums = t_str.rfind('.') + 1;

    //LOG("nums:%s, %d", t_str.c_str(), nums);

    string replace_str = "bin";

    t_str.replace ( nums , 3 , replace_str );

    //LOG("nums:%s, %d", t_str.c_str(), nums);

    mpVocabulary->saveToBinaryFile(t_str);
}
        ...
        :mSensor(sensor),mbReset(false),mbActivateLocalizationMode(false),mbDeactivateLocalizationMode(false)
{
    ...
    LOG("strSettingsFile:%s",strSettingsFile.c_str());
    //Check settings file
...
void System::drawGL(cv::Mat& OImg)
{
    //LOG("drawGL Thread has been started!");

    mpViewer->drawGL(OImg);
}
